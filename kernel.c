...
#error "Not using cross compiler, thats no good"
#endif

#if !defined(__i386__)
#error "This need to be compiled with ix86-elf compiler"
#endif

// Hardware color constant
enum vga_color {
	VGA_COLOR_BLACK = 0,
	VGA_COLOR_BLUE = 1,
	VGA_COLOR_GREEN = 2,
...
void terminal_scroll_down() {
  for (size_t y = 1; y < VGA_HEIGHT; y++) {
    for (size_t x = 0; x < VGA_WIDTH; x++) {
      const size_t index = y * VGA_WIDTH + x;
      const size_t i = (y - 1) * VGA_WIDTH + x;
      char c = terminal_buffer[index] & 0xFF;
      uint8_t color = (terminal_buffer[index] >> 8) & 0xFF;
...
void kernel_main(void) {
  ...
  terminal_writestring_color("test\n", vga_entry_color(VGA_COLOR_BLUE, VGA_COLOR_RED));
}
